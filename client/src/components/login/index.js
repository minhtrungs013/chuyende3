...
import { authActions } from "../../redux/login/index.js";
...
import Swal from "sweetalert2";
 ...
 const BasicForm = () => {
  ...
  const submitHandler = (event) => {
    ...
    loginAPI(entereduserName, enteredPasword)
      .then((res) => {
        const data = res.data;
        dispatch(authActions.login());
        dispatch(authActions.setToken(data.accessToken));
        dispatch(authActions.getid(entereduserName));
      })
      .catch((err) => {
        Swal.fire({
          icon: 'error',
              ...
              <img src={Logo} alt="logo" className="logo" style={{width:"300px"}}></img>
                ...
                <div className="login__import">
                  <div className="form-field">
                    <input type="text" id="name" ref={userNameInputRef} required className="form-input" placeholder=" " />
